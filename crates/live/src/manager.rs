...
impl ExecutionManager {
    ...
    pub async fn reconcile_execution_mass_status(
        ...
        for report in order_reports.values() {
            ...
            if let Some(client_order_id) = &report.client_order_id {
                ...
                let strategy_id_from_db = if is_synthetic {
                    None
                } else {
    ...
    async fn load_reconciliation_strategy_from_db(
            ...
            .contains(&strategy_id)
        {
            ...
            log::debug!(
                "Ignoring database strategy {strategy_id} for reconciled order {venue_order_id}: not in reconciliation_strategy_ids"
            );
            None
        }
    }
    ...
    fn handle_external_order(
        ...
        fills: &[&FillReport],
        is_synthetic: bool,
        strategy_id_from_db: Option<StrategyId>,
...
mod tests {
    ...
    use nautilus_model::{
        ...
        instruments::{Instrument, stubs::crypto_perpetual_ethusdt},
        orders::Order,
    ...
    use super::{ExecutionManager, ExecutionManagerConfig};

    #[test]
    fn test_handle_external_order_prefers_database_strategy_over_claim() {
        ...
        let instrument_id = instrument.id();
        let claimed_strategy_id = StrategyId::from("CLAIMED-1");
        let database_strategy_id = StrategyId::from("FROM-DB");
        let account_id = AccountId::from("BINANCE-001");
        let venue_order_id = VenueOrderId::from("V-001");

        manager.claim_external_orders(instrument_id, claimed_strategy_id);
        ...
        .with_price(Price::from_decimal(dec!(1.0)).unwrap());
        ...
        let (events, metadata) = manager.handle_external_order(
            ...
            &[],
            false,
            Some(database_strategy_id),
        );

        assert!(!events.is_empty());
        assert_eq!(metadata.expect("metadata").strategy_id, database_strategy_id);
        ...
        assert_eq!(order.strategy_id(), database_strategy_id);
    }
}
