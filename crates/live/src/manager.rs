...
use std::{cell::RefCell, fmt::Debug, mem::take, rc::Rc, str::FromStr, sync::LazyLock};
...
pub struct ExecutionManagerConfig {
    ...
    pub reconciliation_strategy_ids: AHashSet<StrategyId>,
...
impl Default for ExecutionManagerConfig {
    fn default() -> Self {
        Self {
            ...
            reconciliation_strategy_ids: AHashSet::new(),
...
impl From<&LiveExecEngineConfig> for ExecutionManagerConfig {
    fn from(config: &LiveExecEngineConfig) -> Self {
        ...
        let reconciliation_strategy_ids: AHashSet<StrategyId> = config
            .reconciliation_strategy_ids
            .clone()
            .unwrap_or_default()
            .into_iter()
            .map(|value| StrategyId::from(value.as_str()))
            .collect();
...
impl ExecutionManager {
    ...
    pub async fn reconcile_execution_mass_status(
        ...
        let mut registered_venue_order_ids: AHashSet<VenueOrderId> = AHashSet::new();
        ...
        for report in order_reports.values() {
            ...
            if let Some(client_order_id) = &report.client_order_id {
                    ...
                    && self.is_exact_order_match(&cached_order, report)
                {
                    ...
                    self.push_open_order_tracking_registration(
                        &mut external_orders,
                        &mut registered_venue_order_ids,
                        &cached_order,
                        report,
                    );
                ...
                if let Some(order) = self.get_order(client_order_id) {
                    ...
                    if let Some(instrument) = self.get_instrument(&report.instrument_id) {
                        // let strategy_id_from_db = self
                        //     .load_reconciliation_strategy_from_db(&report.venue_order_id)
                        //     .await;
                        let order_fills: Vec<&FillReport> = fill_reports
                            .get(&report.venue_order_id)
                            .map(|f| f.iter().collect())
                ...
                let order_from_db = if is_synthetic {
                    None
                } else {
                    self.load_order_from_db(&report.venue_order_id).await
                    // self.load_reconciliation_strategy_from_db(&report.venue_order_id)
                    //     .await
                };
                // log::info!("vid:{},{:?}",report.venue_order_id,strategy_id_from_db);
    ...
    fn load_reconciliation_strategy_from_db(
        &self,
        order:Option<&OrderAny>,
    ) -> Option<StrategyId> {
        if self.config.reconciliation_strategy_ids.is_empty() {
            return None;
        }
        ...
        // log::warn!("{strategy_id},{:?}",self.config.reconciliation_strategy_ids);
        if self
            .config
            .reconciliation_strategy_ids
            .contains(&strategy_id)
        {
            ...
            log::debug!(
                "Ignoring database strategy {strategy_id} for reconciled order {:?}: not in reconciliation_strategy_ids",order.venue_order_id()
            );
            None
        }
    }

    fn push_open_order_tracking_registration(
        &self,
        registrations: &mut Vec<ExternalOrderMetadata>,
        registered_venue_order_ids: &mut AHashSet<VenueOrderId>,
        order: &OrderAny,
        report: &OrderStatusReport,
    ) {
        if !report.order_status.is_open() {
        ...
        if !registered_venue_order_ids.insert(report.venue_order_id) {
            return;
        }

        registrations.push(ExternalOrderMetadata {
    ...
    fn handle_external_order(
        ...
        fills: &[&FillReport],
        is_synthetic: bool,
        order_from_db: Option<OrderAny>,
    ) -> (Vec<OrderEventAny>, Option<ExternalOrderMetadata>) {
        let strategy_id_from_db=self.load_reconciliation_strategy_from_db(order_from_db.as_ref());
        let (strategy_id, tags) =
            if let Some(strategy_id_from_db) = strategy_id_from_db {
                let order_id = report
                    .client_order_id
                    .map_or_else(|| report.venue_order_id.to_string(), |id| id.to_string());
                log::info!(
                    color = LogColor::Blue as u8;
                    "External order {} for {} recovered database strategy {}",
                    order_id,
                    report.instrument_id,
                    strategy_id_from_db,
                );
                let tags=order_from_db.and_then(|o| o.tags().map(|v|v.to_vec()));
                (strategy_id_from_db, tags)
            } else if let Some(claimed_strategy) = self.external_order_claims.get(&report.instrument_id) {
                let order_id = report
                    .client_order_id
                    .map_or_else(|| report.venue_order_id.to_string(), |id| id.to_string());
                log::info!(
